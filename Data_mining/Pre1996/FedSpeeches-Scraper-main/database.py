...
class Database:
    ...
    def get_section(self, table, _type, limit=None):
        query = "SELECT * from {0} where section = '{1}';".format(table, _type)
    ...
    def write_many(self, table, columns, data_list):
        data_tuple_list = [tuple(l) for l in data_list]
        self.cursor.executemany(f'insert into {table} values ({columns})', data_tuple_list)
